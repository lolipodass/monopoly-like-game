...
const changeMoney = (playerId, moneys) => {
  ...
  io.emit('MoneyChanged', playerId, money, players.get(playerId).name);
}
...
io.on('connection', (socket) => {
  ...
  const standOnCompany = (player, company) => {
    ...
    if (cell[0] === -1) {
      ...
      changeMoney(turn, -price);
      changeMoney(cell[0], price);
      readyToEndMove = true;
    }
  }

  const dice = () => {
    // const dice1 = throwDice(6);
    // const dice2 = throwDice(6);
    const dice1 = 1
    const dice2 = 2
